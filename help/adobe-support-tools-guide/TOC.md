---
user-guide-title: Guia de ferramentas e suporte do Adobe
user-guide-description: Orientação e atualizações detalhadas sobre o suporte e as ferramentas da Adobe.
feature-set: Experience Cloud
feature: Support
type: Documentation
index: true
role: User, Admin
source-git-commit: 25fc32de197848e34e84db29113d17689442678f
workflow-type: tm+mt
source-wordcount: '365'
ht-degree: 2%
---
# Guia de ferramentas e suporte do Adobe {#adobe-support-tools-guide}

- [Visão geral do Guia de ferramentas e suporte do Adobe](/help/adobe-support-tools-guide/adobe-support-tools-guide-overview.md)
- [Notas de versão de suporte do Experience League](/help/adobe-support-tools-guide/release-notes.md)
- [experiência de suporte ao cliente do Adobe](adobe-customer-support-experience.md)
- [Configuração de direitos de suporte ao cliente da Adobe](adobe-customer-support-entitlement-configuration.md)
- [Atualização do número de telefone do suporte](phone-numbers.md)
- [Gestão de encaminhamento](/help/adobe-support-tools-guide/escalation-management.md)
- [Insights do sistema](/help/adobe-support-tools-guide/system-insights.md)
- Portal de suporte do Experience League - nova interface de usuário {#exl-new-ui}
  - [Visão geral](/help/adobe-support-tools-guide/exl-new-ui-overview.md)
  - [Criar e gerenciar casos de suporte](/help/adobe-support-tools-guide/exl-new-ui-support-cases.md)
- [Assistente de voz do Adobe Support — Suporte fora do horário de expediente e de fim de semana (Japão)](/help/adobe-support-tools-guide/adobe-support-voice-assistant.md)
- Adobe Admin Console {#adobe-admin-console}
  - [Navegação no Adobe Admin Console](adobe-admin-console/navigating-the-adobe-admin-console.md)
  - [Executar trabalhos pendentes](adobe-admin-console/execute-jobs.md)
  - [Visão geral do Admin Console](adobe-admin-console/admin-console-overview.md)
  - [Funções de administrador](adobe-admin-console/admin-roles.md)
  - [Gerenciar grupos de usuários na Global Admin Console](adobe-admin-console/manage-user-groups.md)
  - [Acessibilidade no Adobe Admin Console](adobe-admin-console/admin-console-accessibility.md)
  - [Alocar produtos a organizações secundárias usando o Global Admin Console](adobe-admin-console/allocate-products.md)
  - [Adotar a administração global](adobe-admin-console/adopt-global-administration.md)
  - [Gerenciar hierarquia da organização](adobe-admin-console/set-up-organizations.md)
  - [Criar relatórios de atribuição de licença para várias organizações e produtos](adobe-admin-console/create-license-assignment-reports-for-multiple-organizations.md)
  - [Selecione uma organização na Global Admin Console](adobe-admin-console/select-an-organization-in-the-global-admin-console.md)
  - [Gerenciar administradores](adobe-admin-console/manage-administrators.md)
  - [Gerenciar perfis de produto no Global Admin Console](adobe-admin-console/manage-product-profiles.md)
  - [Atualizar políticas da organização na Global Admin Console](adobe-admin-console/update-policies.md)
  - [Gerenciar modelos de política no Global Admin Console](adobe-admin-console/manage-policy-templates.md)
  - [Exportar ou importar estrutura da organização e alocações de produtos](adobe-admin-console/export-or-import-organization-structure-and-product-allocations.md)
  - [Forçar a expiração da senha no Admin Console](adobe-admin-console/enforce-password-expiration-in-the-admin-console.md)
  - [Configurar identidade e logon único](adobe-admin-console/adobe-set-up-identity-and-sso.md)
  - [Usuários do Adobe Admin Console](adobe-admin-console/adobe-admin-console-users.md)
  - [Visão geral da identidade](adobe-admin-console/adobe-identity-overview.md)
  - [Limitar o acesso ao produto por endereços IP](adobe-admin-console/adobe-ip-based-access.md)
  - [Baixar logs de auditoria e exportar relatórios](adobe-admin-console/download-audit-logs-and-export-reports.md)
  - [Migrar usuários existentes para o Adobe Admin Console](adobe-admin-console/migrate-existing-users-to-the-adobe-admin-console.md)
  - [Migrar o gerenciamento de usuários do Frame.io para o Admin Console](adobe-admin-console/migrate-user-management-to-the-adobe-admin-console.md)
- Suporte ao Adobe Workfront {#workfront}
  - [Perguntas frequentes](faq.md)
- Suporte ao Adobe Commerce {#adobe-commerce-support}
  - [Visão geral de suporte do Adobe Commerce](adobe-commerce-support/adobe-commerce-support-overview.md)
  - [Aviso de fim de suporte do MySQL e orientação de compatibilidade de banco de dados para o Adobe Commerce](adobe-commerce-support/mysql-end-of-support-notice-and-database-compatibility-guidance-for-adobe-commerce.md)
  - [Como solicitar o Adobe Commerce temporário no upsize da infraestrutura em nuvem](adobe-commerce-support/how-to-request-temporary-adobe-commerce-on-cloud-infrastructure-upsize.md)
  - [Solicitações de capacidade de aumento temporário do Adobe Commerce em nossa infraestrutura em nuvem](adobe-commerce-support/holiday-surge-capacity-requests-for-magento-commerce-cloud.md)
  - [Linha direta de notificação Adobe Commerce P1](adobe-commerce-support/adobe-commerce-p1-notification-hotline.md)
  - [Adobe Commerce Intelligence Essentials: atualizar a forma de pagamento ou cancelar a assinatura](adobe-commerce-support/adobe-commerce-intelligence-essentials-update-payment-cancel-subscription.md)
  - [Atualização da política de ciclo de vida do tíquete de suporte da Adobe Commerce](adobe-commerce-support/magento-support-ticket-lifecycle-policy-update.md)
  - [Folha de fatos de monitoramento do Adobe Commerce na infraestrutura do Cloud Pro](adobe-commerce-support/monitoring-whitepaper.md)
  - [Perguntas frequentes sobre o fim do suporte ao software Adobe Commerce](adobe-commerce-support/adobe-commerce-eos-policy-faq.md)
  - [Descrições do motivo do problema com o tíquete de suporte da Adobe Commerce](adobe-commerce-support/adobe-commerce-support-ticket-contact-reason-descriptions.md)
  - [Principais características das ferramentas de suporte do Adobe Commerce](adobe-commerce-support/support-tools-overview.md)
  - [Guia do usuário de suporte da Experience League para Adobe Commerce](adobe-commerce-support/adobe-commerce-help-center-user-guide.md)
  - [Como incluir um membro da equipe nas notificações de suporte](/help/adobe-support-tools-guide/adobe-commerce-support/how-to-include-a-team-member-in-support-notifications.md)
  - [Solicitações de sandbox da infraestrutura em nuvem do Adobe Commerce para parceiros](adobe-commerce-support/partner-magento-commerce-cloud-sandbox-requests.md)
  - [Não foi possível adicionar o usuário ao projeto na nuvem do Adobe Commerce](adobe-commerce-support/unable-add-user-adobe-commerce-cloud-project.md)
  - [Como aplicar um patch de compositor fornecido pelo Adobe](adobe-commerce-support/how-to-apply-a-composer-patch-provided-by-magento.md)
  - [Como aplicar um patch isolado fornecido pelo Adobe](adobe-commerce-support/how-to-apply-an-isolated-security-patch-file-provided-by-adobe.md)
  - [Visão geral das ferramentas de suporte](adobe-commerce-support/overview.md)
